...
prop_eval_subs_wrongargs :: Integer -> Bool
prop_eval_subs_wrongargs a = case eval (List [Atom "-", Number a]) of
        Left (NumArgs 2 _) -> True
        _ -> False

-- r6rs, however, supports 0 or more args for + and *.  zero args
...
prop_eval_mul1 :: Integer -> Bool
prop_eval_mul1 a = case eval (List [Atom "*", Number a]) of
        Right (Number b) -> a == b
        _ -> False

prop_eval_mul0 :: Bool
prop_eval_mul0 = case eval (List [Atom "*"]) of
        Right (Number 1) -> True
        _ -> False
...
testEval = testGroup "Eval"
    ...
    ,   testProperty "Substract one int (wrong args)" prop_eval_subs_wrongargs
    ,   testProperty "mult one int" prop_eval_mul1
    ,   testProperty "mult zero ints" prop_eval_mul0
    ]
